odoo.define("oneshare.tightening_image_editor", function (require) {
  ...
  var OneshareTighteningImageEditor = OneshareImageEditor.extend({
        ...
        $.ajax({
          ...
          .done(function (resp) {
            self._notifyInfo(
              "success",
              _t("Success"),
              _t("Save Tightening Points Success!"),
              false
            );
          ...
          .fail(function (resp) {
            self._notifyInfo(
              "danger",
              _t("Error"),
              _t("Save Tightening Points Error!"),
              false
            );
          });
      } catch (e) {
    ...
    inline_add_new_mask: function (top, left) {
      ...
      var topOffset = top || 0;
      var t = _.str.sprintf(
        '<div class="oe_mark_circle">%s</div>',
        _.str.escapeHTML(this.markPoints.length + 1)
      );
      var e = $(t)
        .css({
          left: leftOffset
            ? "calc(" + leftOffset + "% - 10px)"
            : "calc(" + leftOffset + "%)",
          top: topOffset
            ? "calc(" + topOffset + "% - 10px)"
            : "calc(" + topOffset + "%)",
      ...
      this.markPoints.push({
        sequence: this.markPoints.length + 1,
        x_offset: left || 0,
        y_offset: top || 0,
      });
    },
    ...
    dragMoveListener: function (event) {
      ...
      var currentX =
        (parseFloat(target.getAttribute("current-data-x")) || 0) + event.dx;
      var currentY =
        (parseFloat(target.getAttribute("current-data-y")) || 0) + event.dy;
      var totalX =
        (parseFloat(target.getAttribute("total-data-x")) || 0) + event.dx;
      var totalY =
        (parseFloat(target.getAttribute("total-data-y")) || 0) + event.dy;
    ...
    MarkerDragstop: function (event) {
      ...
      self.markPoints[idx].x_offset =
        self.markPoints[idx].x_offset === 0
          ? imgWidth
            ? ((x + circleWidth / 2) / imgWidth) * 100
            : 0
          : imgWidth
          ? self.markPoints[idx].x_offset + (x / imgWidth) * 100
          : 0;
      self.markPoints[idx].y_offset =
        self.markPoints[idx].y_offset === 0
          ? imgHeight
            ? ((y + circleHeight / 2) / imgHeight) * 100
            : 0
          : imgHeight
          ? self.markPoints[idx].y_offset + (y / imgHeight) * 100
          : 0;
  ...
  fieldRegistry.add(
    "oneshare_tightening_image_editor",
    OneshareTighteningImageEditor
  );
